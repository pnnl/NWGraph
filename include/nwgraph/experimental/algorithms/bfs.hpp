...
template <adjacency_list_graph Graph>
size_t BU_step(const Graph& g, std::vector<vertex_id_t<Graph>>& parents, nw::graph::AtomicBitVector<>& front,
               nw::graph::AtomicBitVector<>& next) {
...
size_t TD_step(const Graph& g, std::vector<vertex_id_t<Graph>>& parents, Vector& cur, std::vector<Vector>& next) {
...
[[gnu::noinline]] auto bfs_v1(const OutGraph& out_graph, const InGraph& in_graph, vertex_id_t<OutGraph> root, int num_bins = 32, int alpha = 15,
                              int beta = 18) {
  ...
  while (!queue.empty()) {
    if (scout_count > edges_to_check / alpha) {
      ...
      scout_count = tbb::parallel_reduce(
          tbb::blocked_range(0ul, queue.size()), 0ul,
...
template <adjacency_list_graph OutGraph, adjacency_list_graph InGraph>
[[gnu::noinline]] auto bfs_v2(const OutGraph& out_graph, const InGraph& in_graph, vertex_id_t<OutGraph> root, int num_bins = 32, int alpha = 15,
                              int beta = 18) {
