...
class vector_of_list_of_structs : public std::vector<std::list<std::tuple<Attributes...>>> {

public:
  using base  = std::vector<std::list<std::tuple<Attributes...>>>;
  using inner = std::list<std::tuple<Attributes...>>;
...
public:
  ...
  index_adj_list(edge_list<directedness::directed, Attributes...>& A) : base(A.num_vertices()[0]) {
    num_edges_ = fill_adj_list(A, *this);
  }

  index_adj_list(edge_list<directedness::undirected, Attributes...>& A) : base(A.num_vertices()[0]) {
    num_edges_ = fill_adj_list(A, *this);
  }
  ...
  void push_back(size_t i, size_t j, Attributes... attrs) {
    ...
    base::operator[](i).emplace_back(j, attrs...);
  }
