...
std::size_t power_gen(double dist_min, double dist_max) {
  ...
  return (std::size_t)pow((max_pow - min_pow) * drand48() + min_pow, 1 / (exponent + 1));
}
static double standard_exponential_unlikely(bitgen_t* bitgen_state, uint8_t idx, double x) {
  if (idx == 0) {
    /* Switch to 1.0 - U to avoid log(0.0), see GH 13361 */
    return ziggurat_exp_r - log(1.0 - next_double(bitgen_state));
  } else if ((fe_double[idx - 1] - fe_double[idx]) * next_double(bitgen_state) + fe_double[idx] < exp(-x)) {
    return x;
  } else {
...
*/
    template <class T>
    nw::graph::edge_list<nw::graph::undirected> configuration_model(std::vector<T>& deg_seqa, std::vector<T>& deg_seqb,
                                                                    bool contiguous_id_space = false) {
