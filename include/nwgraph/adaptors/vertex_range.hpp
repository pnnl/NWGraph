...
public:
  ...
  difference_type operator-(const counting_iterator& it) const {
    return my_counter_ - it.my_counter_;
  }
  ...
  counting_iterator operator+(difference_type forward) const {
    return counting_iterator(my_counter_ + forward);
  }
  counting_iterator operator-(difference_type backward) const {
    return counting_iterator(my_counter_ - backward);
  }
...
public:
  vertex_range(vertex_id_type max) : begin_(counting_iterator<vertex_id_type>(0)), end_(counting_iterator<vertex_id_type>(max)) {
  }

  vertex_range(const vertex_range&) = default;
  vertex_range(vertex_range&&)      = default;

  vertex_range(vertex_range& rhs, tbb::split) : begin_(rhs.begin_), end_(rhs.end_) {
  }

  counting_iterator<vertex_id_type> begin() const {
    return begin_;
  }
  counting_iterator<vertex_id_type> end() const {
    return end_;
  }
